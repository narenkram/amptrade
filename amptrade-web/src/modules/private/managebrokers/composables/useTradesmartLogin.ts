...
export function useTradesmartLogin() {
  const error = ref<string | null>(null)
  const isTradesmartLoggingIn = ref<boolean>(false)
  const showLoginModal = ref<boolean>(false)
  const loginForm = ref<TradesmartLoginForm>({
    password: '',
    totp: '',
  })
  ...
  const handleLogin = async (
    ...
    try {
      isTradesmartLoggingIn.value = true
      error.value = null
      ...
      const response = await api.post(`${import.meta.env.VITE_API_URL}/tradesmart/login`, {
        jKey: broker.apiKey,
