...
export function useShoonyaLogin() {
  const error = ref<string | null>(null)
  const isShoonyaLogginIn = ref<boolean>(false)
  const showLoginModal = ref<boolean>(false)
  const loginForm = ref<ShoonyaLoginForm>({
    password: '',
    totp: '',
  })
  ...
  const handleLogin = async (
    ...
    try {
      ...
      const response = await api.post(`${import.meta.env.VITE_API_URL}/shoonya/login`, {
        jKey: broker.apiKey,
